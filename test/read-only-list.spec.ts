...
describe('./read-only-list.ts', function () {
  ...
  it('ReadOnlyList.SequenceEqual 方法正常运作', function () {
    let list = new ReadOnlyList(new List([1, 2, 3, 1, 2]))
    expect(list.SequenceEqual(new ReadOnlyList(new List([2, 3, 4, 5, 3])))).is.false
    expect(list.SequenceEqual(new ReadOnlyList(new List([1, 2, 3, 1, 2])))).is.true
  })
  ...
  it('ReadOnlyList.SingleOrDefault 方法正常运作', function () {
    let list = new ReadOnlyList(new List<number>([]))
